...
import type {
  Attribution, FindingName, ItemLink, VerifiedChangeset, VerifiedChangesetV1, VerifiedChangesetV2
} from './types.js';

/**
 * Proves an artifact agrees with itself: its derived fields (references, findings,
 * attribution, links, summary, verdict) follow from the primitives it records.
 * Version-routed — a v1 artifact is checked under v1 semantics, a v2 artifact under
 * ADR 0008 semantics. This is not repository verification; see `verify-git.ts`.
 */
export function assertVerifiedSemantics(verified: VerifiedChangeset): void {
  if (verified.version === 1) return assertVerifiedSemanticsV1(verified);
  return assertVerifiedSemanticsV2(verified);
}

function assertItemsMatchChangeset(
  verified: VerifiedChangeset, problems: string[]
): void {
...
function assertVerifiedSemanticsV1(verified: VerifiedChangesetV1): void {
  ...
  const recomputed = summariseV1(verified);
  ...
  assertVerdict(verified, problems);
  finish(problems);
}

function assertVerifiedSemanticsV2(verified: VerifiedChangesetV2): void {
  const problems: string[] = [];
  ...
  const divergentRepos = new Set(
    verified.ranges.filter((r) => !r.baseIsAncestorOfHead).map((r) => r.repo)
  );
  const divergentInRun = divergentRepos.size > 0;

  const derivedLinks = new Map<string, ItemLink[]>();
  for (const item of verified.items) derivedLinks.set(item.id, []);
  ...
  for (const commit of verified.commits) {
    ...
    if (commit.attribution !== attribution) {
    ...
    for (const ref of commit.references) {
      for (const id of ref.resolvesTo) {
        if (!itemIds.has(id)) {
          ...
          const bucket = derivedLinks.get(id)!;
          if (!bucket.some((c) => c.repo === commit.repo && c.sha === commit.sha)) {
            bucket.push({ repo: commit.repo, sha: commit.sha, attribution: commit.attribution });
          }
        }
      }
    }

    // A divergent commit emits no negative attribution regardless of its references.
    const expected: FindingName[] = commit.ignored !== null || divergent
      ? []
      : commit.references.length === 0
        ? ['no-reference']
        : commit.references.some((r) => r.resolvesTo.length === 0) ? ['unknown-reference'] : [];
  ...
  for (const item of verified.items) {
    ...
    const hasDeterminate = item.commits.some((l) => l.attribution === 'determinate');
    let expectedAttribution: Attribution;
    let expectedFindings: FindingName[];
    if (hasDeterminate) {
      expectedAttribution = 'determinate';
      expectedFindings = [];
    } else if (item.commits.length === 0 && !divergentInRun) {
      expectedAttribution = 'determinate';
      expectedFindings = ['item-without-commits'];
    } else {
      expectedAttribution = 'indeterminate';
      expectedFindings = [];
    }
    if (item.attribution !== expectedAttribution) {
      problems.push(`item "${item.id}" declares attribution "${item.attribution}" but its links and the run imply "${expectedAttribution}"`);
    }
    if (canonicalStringify(item.findings) !== canonicalStringify(expectedFindings)) {
      problems.push(`item "${item.id}" declares findings [${item.findings.join(', ')}] but its links and the run imply [${expectedFindings.join(', ')}]`);
    }
  }
...
function assertVerdict(verified: VerifiedChangeset, problems: string[]): void {
...
function finish(problems: string[]): void {
